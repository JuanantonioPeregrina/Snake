...
//Como vamos a usar una variable de entorno  comentamos el secret
app.use(session({ // Configuración de la sesión (Guardar datos de usuario en el servidor)
  //secret: "Una frase muy secreta",
  secret: process.env.SESSION_SECRET || "Una frase muy secreta",
  resave: false,
  saveUninitialized: true
}));

// Define las dimensiones del tablero y funciones globales
global.BOARD_SIZE = 10; // Tablero de 10x10

// Convertir coordenadas a índice único
global.getUniqueIndex = (x, y) => y * BOARD_SIZE + x;

// Convertir índice único a coordenadas (x, y)
global.getCoordinates = (index) => ({
  x: index % BOARD_SIZE,
